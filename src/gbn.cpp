...
int A_nextseqnum;	   // next seq number for A
...
int B_expseqnum;		   // seq number for b
...
void A_output(struct msg message) //ram's comment - students can change the return type of the function from struct to pointers if necessary
{
	...
	A_buffer[A_position].isSent=false;
	/**************Send Packet**************/
	if(A_nextseqnum < (A_base+WINSIZE))
	{
		//cerr<<"/////Stored at "<<A_position<<endl;
		/**************Send if seq falls in window**************/
		if (A_buffer[A_nextseqnum].isSent==false && A_buffer[A_nextseqnum].isSet==true)
		{
			tolayer3(0,A_buffer[A_nextseqnum].packet);
			A_buffer[A_nextseqnum].isSent=true;
			A_transport++;
			//cerr<<"/////A Sent Packet "<<A_transport<<" Seq Num "<<A_buffer[A_nextseqnum].packet.seqnum<<endl;
		}
...
{
	/**************ACK Validator**************/
	if(packet.acknum>=A_base && notCorrupt(packet)==true)
	{
		//cerr<<"/////A got valid Ack "<<packet.acknum<<endl;
		...
		for (int i = A_nextseqnum; i < A_base+WINSIZE; ++i)
		{
			if (A_buffer[i].isSent==false && A_buffer[i].isSet==true)
			{
				tolayer3(0,A_buffer[i].packet);
				A_buffer[i].isSent=true;
				A_transport++;
				//cerr<<"/////A Sent Packet "<<A_transport<<" Seq Num "<<A_buffer[i].packet.seqnum<<endl;
				if(A_base==i)
				{
					starttimer(0,TIMEOUT);
				}
	...
	else
	{
		...
		{
			//cerr<<"!!!!!Corrupt Ack Received at A "<<endl;
			//cerr<<"..."<<endl;
		}
		else
		{
			//cerr<<"!!!!!Invalid Ack Received at A "<<packet.acknum<<endl;
			//cerr<<"..."<<endl;
		}
	}
}
...
{
	/**************If Timeout Occurs Send All the Window Again**************/
	//cerr<<"!!!!!Timeout at A. Re-Sending Window"<<endl;
	for (int i=A_base; i<A_base+WINSIZE && A_buffer[i].isSent==true && A_buffer[i].isSet==true ; i++)
	{
		//cerr<<"!!!!!A Sent Packet "<<A_transport<<" Seq Num "<<A_buffer[i].packet.seqnum<<endl;
		tolayer3(0,A_buffer[i].packet);
		A_transport++;
...
{
	A_nextseqnum=1;	   // next seq number for A
...
{
	B_transport++;
	/**************Received Packet Validator & Acker**************/
	if(packet.seqnum==B_expseqnum && notCorrupt(packet)==true)
	{
		//cerr<<"/////B got valid Packet "<<packet.seqnum<<endl;
		B_application++;
		tolayer5(1,packet.payload);
		struct pkt B_ackpacket;
		B_ackpacket.acknum=B_expseqnum;
		B_ackpacket.seqnum=B_expseqnum;
	...
	{
		/**************Display Problem with Received Packet and Send Previous ACK**************/
		if(notCorrupt(packet)==true)
		{
			//cerr<<"!!!!!B got invalid packet, SeqNum "<<packet.seqnum<<endl;
		}
		else
		{
			//cerr<<"!!!!!B got corrupt packet "<<endl;
		}
		tolayer3(1,B_lastack);
		//cerr<<"!!!!!Sent previous Ack "<<endl;
	}
	//cerr<<"..."<<endl;
}
